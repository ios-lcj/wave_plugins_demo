...
const IndexPage = () => {
    ...
    const setUserConfig = () => {
        ...
        // @ts-ignore
        pluginSDK.userConfig.addUserConfig({ userConfig: JSON.stringify(userConfig) }, function ({ errorCode }: { errorCode: number }) {
            if (errorCode === 0) {
                // @ts-ignore
    ...
    const makeP2PAudioCall = () => {
        // @ts-ignore
        pluginSDK.call.makeP2PAudioCall({ callNumber: '1111' }, function ({ errorCode }) {
            if (errorCode === 0) {
                // @ts-ignore
    ...
    const makeP2PVideoCall = () => {
        // @ts-ignore
        pluginSDK.call.makeP2PVideoCall({ callNumber: '1111' }, function ({ errorCode }) {
            if (errorCode === 0) {
                // @ts-ignore
                pluginSDK.log.log('MakeP2PAudioCall success');
            }
    ...
    useEffect(() => {
         ...
         */
        // @ts-ignore
        pluginSDK.eventEmitter.on('onRecvP2PIncomingCall', function ({ callType, callNum }) {
            // 打印log日志
            // @ts-ignore
         ...
         */
        // @ts-ignore
        pluginSDK.eventEmitter.on('onInitP2PCall', function ({ callType, callNum }) {
            // 打印info日志
            // @ts-ignore
         ...
         */
        // @ts-ignore
        pluginSDK.eventEmitter.on('onRejectP2PCall', function ({ callType, callNum }) {
            // 打印warn日志
            // @ts-ignore
         ...
         */
        // @ts-ignore
        pluginSDK.eventEmitter.on('p2PCallCanceled', function ({ callType, callNum }) {
            // error
            // @ts-ignore
